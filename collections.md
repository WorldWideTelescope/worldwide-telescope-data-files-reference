...
| ```DataSetType="Sky"``` | One of:**Sky**, **Planet**, **Earth**, **Panorama**, **Survey**.<br>Note that images are currently only rendered in **Sky** mode. |
| ```BandPass="Visible"``` | The primary wavelength. One of: **Gamma**, **XRay**, **Ultraviolet**, **Visible**, **HydrogenAlpha**, **IR**, **Microwave**, **Radio**.<br>Note that only one entry can be made, even if the image is a composite of several wavelengths. |
| ```Url="_path._png"``` | A URL that contains a link to the 256 x 256 tile pyramid  of the image. In this case, it is a path to an image on the web, but it can be any URL (for example, a local path or a SQL query) that contains the size of the pyramid coded in the {} parameters. This would be a path similar to: <br> ```http://research.microsoft.com/...../530919080/{2}/{3}/{3}_{0}.png``` |
